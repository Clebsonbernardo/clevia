/*
# Fix infinite recursion in company_members RLS policies (re-apply)
...
This migration was previously written to disk but never applied to the
database. Re-applying now.

## Fix

### 1. SECURITY DEFINER helper functions
- `is_company_member(company_id, user_id)` — returns true if the given user
  is a member of the given company. Runs as the owner (SECURITY DEFINER) so
  it bypasses RLS and breaks the recursion.
- `is_company_ceo(company_id, user_id)` — returns true if the given user is a
  `ceo` member of the given company. Also SECURITY DEFINER.
...
All are STABLE, set search_path to public, and only read — no side effects.
...
- update_members: company owner OR existing CEO of that company (via helpers).
- delete_members: company owner OR existing CEO of that company (via helpers).
